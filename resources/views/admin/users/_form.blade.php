...
<div>
    ...
    <input name="email" type="email" value="{{ old('email', $currentUser->email ?? '') }}" class="w-full rounded-2xl border border-emerald-100 bg-emerald-50 px-4 py-3 outline-none focus:border-emerald-300 focus:bg-white" required>
</div>
<div>
    <label class="mb-2 block text-sm text-slate-600">Password {{ $currentUser ? '(optional)' : '' }}</label>
    <input name="password" type="password" class="w-full rounded-2xl border border-emerald-100 bg-emerald-50 px-4 py-3 outline-none focus:border-emerald-300 focus:bg-white" {{ $currentUser ? '' : 'required' }}>
</div>
<div>
